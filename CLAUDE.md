...
## 데이터 정책 (중요)
- **자료 수집은 API 키를 쓰지 않는다.** Claude가 공개 페이지·문서에 직접 붙어 읽고 구조화한다.
  자바스크립트로 그려지는 사이트는 브라우저형 수집(Claude in Chrome)이 필요.
- **API 키는 오직 법령 조문 조회(법제처 OC 인증)에만** 쓴다. → `docs/legal-api.md`
