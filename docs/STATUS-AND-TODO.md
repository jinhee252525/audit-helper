...
## P0. 업무 한 건 완주 (2026-09-09)

- **히어로 P0 landed** (`app/index.html` · `app/app.js` · `app/styles.css`)
  - 자료범위 배지(실인덱스/데모/폴백·건수) 상시 표시. 데모·폴백 시 **실무용 확정·출처포함 내보내기 잠금**(초안 미리보기만 + 배너).
  - 상황 제출 후 최소 보완 프롬프트: 기관유형 · 업무단계(선택) · 대략 시기 — 값 미입력은 **미확인**(추정 채움 없음).
  - 선례 카드: 왜 매칭 / 나와의 차이 / 원문 가능(URL·발췌) / 처분 라벨(`처분 미확인`·`원문에 처분 기재 없음`·`해당 없음`·`미부과 확인` + 실처분문).
  - 체크박스 선택 근거만 메모 반영. 메모에 출처 URL·수집일·미확인 사항 섹션.
  - 원문 온디맨드: 새 탭 URL + 발췌 패널(Pages 대리수집 불가 안내). 로컬 findings.json 있으면 필드 보강.
  - 출처 포함 복사 · 실패 UI(결과없음/목록만/비교어려움/검색오류) · 입력 프라이버시 안내(로컬 처리).
  - 실인덱스 있으면 샤드 검색, 없으면 hero-sample 데모.
- **B1 각주(소)**: status 「수집 자료 내 증가」+ small_n 주의.
- **명시적 OUT 유지**: 학교 role-lens 해제 · 대량 본문 crawl · 전체 메모 영속(P1) · 임베딩/CF · 대시보드 B2/D.
